    ...
    def creating_session(self):
        ...
        if self.round_number == 1:
            if Constants.num_paying_rounds > 0:
                ...
                paying_rounds.sort()
                self.session.vars['lottery_paying_rounds'] = paying_rounds

            for p in self.get_players():
                p.participant.vars['lottery_payoff_info'] = []
        ...
        paying_round = self.round_number in self.session.vars['lottery_paying_rounds']
...
class Player(BasePlayer):
    ...
    time_spent = models.PositiveIntegerField() # current round in set
    ...
    def set_payoffs(self):
        if self.paying_round: # if the current round is payoff relevant
            ...
            # ------------------------------------------------------------------------------------------------------------
            self.participant.vars['lottery_payoff_info'].append([self.set_number, self.round_in_set,
                                                             self.x,self.px,self.y, sure_payoff, choice_to_pay, self.payoff])
            if 'lottery%d_payoff'%self.set_number in self.participant.vars:
                self.participant.vars['lottery%d_payoff'%self.set_number] += self.payoff
            else:
                self.participant.vars['lottery%d_payoff'%self.set_number] = self.payoff

            print(self.id_in_group, self.participant.vars['lottery_payoff_info'])
