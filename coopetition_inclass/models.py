...
class Constants(BaseConstants):
    name_in_url = 'coopetition_inclass'
    instructions_template = 'coopetition_inclass/Instructions_template.html'
    breakdowns_template = 'coopetition_inclass_quiz/Instructions_template.html'
    history_template = 'coopetition_inclass//History.html'
    historyall_template = 'coopetition_inclass//HistoryAllRounds.html'
...
class Group(BaseGroup):
    def interact(self):
        ...
        if p1.timed_out and (not self.session.config['debug']):  ## still assign earnings for timed out subjects when debugging
            p1.payoff = 0
        else:
            p1.payoff = p1.potential_payoff
        if p2.timed_out and (not self.session.config['debug']):
            p2.payoff = 0
        else:
